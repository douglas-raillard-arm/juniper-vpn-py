...
class tncc(object):
    ...
    def gen_funk_response(self, certs):
        ...
        msg = b"<FunkMessage VendorID='2636' ProductID='1' Version='1' Platform='%s' ClientType='Agentless'> " % plaform
        msg += b"<ClientAttributes SequenceID='0'> "
        msg += b"<Attribute Name='Platform' Value='%s' />" % platform
        for name, value in certs.items():
            certmsg = b"<Attribute Name='%s' Value='%s' />" % (name.encode('ascii'), value.data.encode('ascii'))
            msg += certmsg + certmsg
        msg += b"</ClientAttributes>  </FunkMessage>"
