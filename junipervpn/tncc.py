...
class TNCC:
    ...
    def gen_funk_platform(self):
        ...
        msg = b"<FunkMessage VendorID='2636' ProductID='1' Version='1' Platform='%s'> " % self.platform.encode('ascii')
    ...
    def gen_funk_response(self, certs):
        ...
        platform = self.platform.encode('ascii')
        msg = b"<FunkMessage VendorID='2636' ProductID='1' Version='1' Platform='%s'> " % plaform
        msg += b"<ClientAttributes SequenceID='0'> "
        msg += b"<Attribute Name='Platform' Value='%s' />" % platform
        for name, value in certs.items():
            msg += b"<Attribute Name='%s' Value='%s' />" % (name.encode('ascii'), value.data.encode('ascii'))
        msg += b"</ClientAttributes>  </FunkMessage>"
    ...
    def get_cookie(self, dspreauth=None, dssignin=None):
        ...
        inner = encode_0ce7(b'policy request', MSG_POLICY)
